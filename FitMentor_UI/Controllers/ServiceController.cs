...
namespace FitMentor_UI.Controllers
{
    public class ServiceController : Controller
    {
        ...
        public async Task<IActionResult> CreateService(CreateServiceDto createServiceDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:44339/api/Services", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"https://localhost:44339/api/Services/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
